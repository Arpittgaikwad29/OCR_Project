...
# Google API key setup for agents 2-6
GOOGLE_API_KEY = "AIzaSyC_yR_Z1BNvkL1ixrejs2aqvGHSXK2CwvE"
os.environ["GOOGLE_API_KEY"] = GOOGLE_API_KEY
configure(api_key=GOOGLE_API_KEY)

# Mistral API key setup for Agent 1 (OCR)
MISTRAL_API_KEY = "r64hFVBftXBZmUf9RZjWIEbvgS4Dc4hF"
...
# Initialize Gemini model for other agents
flash_model = GenerativeModel("gemini-2.0-flash")

def call_mistral_api(image, prompt):
    """Helper function to call Mistral API with image"""
    ...
    # Prepare the payload with the image - Fixed structure
    data = {
...
def agent_ocr(image):
    """Agent 1: Extract text from image using OCR with Mistral's pixtral-12b-2409 model"""
    st.write("### Agent 1: OCR Text Extraction (Mistral)")
    with st.spinner("Extracting text from image using Mistral's pixtral-12b-2409..."):
        prompt = "Only extract the exact text content from the image. Don't include any explanation or additional information."
        extracted_text = call_mistral_api(image, prompt)
